...
CONFIG = {
    ...
    "no._of_bins": 5, # this has to be shared since its in BSE file.
...
## Function Approximation Specific Parameters : ##
    ...
    # Observation space 
    "order" : True,   # 2, for seller or buyer and price    This will always be true
    "best": False,    # 2, for ask and bid 
    "worst": False,   # 2, for ask and bid
    "average": False, # 2, for ask and bid
    ...
    # Mapping of keys to feature contributions
    ...
    n_obs_features = sum(feature_contributions[key] for key in keys_to_check if CONFIG_FA.get(key, False))
    CONFIG["n_features"] = n_obs_features + 1 # this is the length of an observation element (plus 1 for action) 
